...
def draw_corners(img,corners):
    for corner in corners:    
        cv2.circle(img,(corner[0],corner[1]),10,(255,255,0),thickness=-1)       

def axis(img, corners, ids):
    ...
    if len(corners) > 0:
        for i in range(0, len(ids)):
            if 1 in ids:
                ...
                index = list(ids).index(1)
                corner_points = corners[index][0]
                corner_index = np.argmax(corner_points.sum(axis=1))
                corner_position = corner_points[corner_index]
                rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(corners[index], 0.08, intrinsic_camera,
                                                                                distortion)

                cv2.aruco.drawAxis(img, intrinsic_camera, distortion, rvec, tvec, 0.2)
...
def draw_numbers(img,corners,ids):
    ...
    thickness = 4
    for corner in corners:    
        cv2.putText(img,str(ids[number]),(corner[0]+10,corner[1]+10), font, 2,(255,0,0),thickness)
        number=number+1
...
def show_spec(img,corners):
    ...
    cv2.putText(img,spec_string,(15,15), font, 0.5,(0,255,0),thickness)
...
def draw_field(img,corners,ids):   #only when 4 ID's are available
    if len(corners)==4:
        ...
        overlay = img.copy()
        cv2.fillPoly(overlay, pts =[contours], color=(255,215,150))
